...
public class ArrayUtils {
    ...
    /**
     * @param sequence the array of bytes to return as an array of doubles
     * @return double[i] = sequence[i] / Constants.kFloatToByteShift cast to a byte
     * Note!: This doesn't handle cast/conversion issues, so don't use this unless you understand the code
     */
    public static double[] getDoubleArrayfromByteArray(byte[] sequence) {
    	double[] result = new double[sequence.length];
    	for (int i = 0; i < result.length; i++) {
    		result[i] = (double)(sequence[i]/(double)Constants.kFloatToByteShift);
    	}
    	return result;
    }
    
    /**
     * @param src the array of doubles to get the slice of
     * @param begin the starting index of the slice
     * @param end the ending index of the slice
     * @return new sliced array
     */
    public static double[] getSlice(double[] src, int begin, int end) {
    	double slice[] = new double[end - begin];
